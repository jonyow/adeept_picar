...
model_folder = "./jj_detector"

def populate_class_labels():
    model_classes = os.path.join(model_folder, "data_classes.txt")

    f = open(model_classes, 'r')
...
def detect_jj(image, confidence=0.5, nms_thresh=0.3, model='yolov3', enable_gpu=False):
    ...
    if model == 'yolov3-tiny':
        ...
        config_file_name =  os.path.join(model_folder, 'yolov3.cfg')
        weights_file_name = os.path.join(model_folder, 'xxxx.h5')  # 'yolov3.weights'  ### PROBLEM my output is h5, but need to .weights file
